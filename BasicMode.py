...
class Controller:
    ...
    my_media = vlc.MediaPlayer(r"C:\Users\lKK97\Dropbox\PC\Downloads\Tame Impala - Borderline.mp3")
    music_playing = False
    """
    Executes commands according to detected gestures.
    ...
    def playmusic():
        if not Controller.v_flag:
            Controller.v_flag=True
            Controller.my_media.play()
            Controller.music_playing = True

    def pausemusic():
        if Controller.music_playing:
            Controller.my_media.pause()
            Controller.music_playing = False
    ...
    def stopmusic():
        Controller.my_media.stop()
        Controller.music_playing = False
        if Controller.v_flag:
    ...
    def handle_controls(gesture, hand_result):  
        ...
        # index_flag reset
        if gesture != Gest.INDEX and Controller.index_flag:
            Controller.index_flag = False

        if gesture != Gest.FIST and Controller.fistflag:
        ...
        if gesture == Gest.INDEX:
            if not Controller.index_flag:
                Controller.index_flag = True
        ...
        elif gesture == Gest.TWO_FINGER_CLOSED and Controller.music_playing:
